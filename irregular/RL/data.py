...
def process_irregular_data(input, preload):
    ...
    training_samples = all_training_samples
    data = {}
    adj_train_orig = adj_data[0:training_samples]
    adj_train_n, data["support_train"] = preprocesss_adj(adj_train_orig)
    data["adj_train"] = adj_train_orig
    ...
    data["degree_train"] = degree_matrix(adj_train_orig)
    data["dist_train"] = dist_data[:training_samples]
    data["coord_matrix_train"] = coord_matrix(data["coord_train"])

    adj_test_orig= adj_data[training_samples:]
    adj_test_n, data["support_test"] = preprocesss_adj(adj_test_orig)
    data["adj_test"] = adj_test_orig
    ...
    data["degree_test"] = degree_matrix(adj_test_orig)
    data["dist_test"] = dist_data[training_samples:]
    data["coord_matrix_test"] = coord_matrix(data["coord_test"])
    
    adj_train_n = np.array([adj_train_orig[i].toarray() for i in range(adj_train_n.shape[0])])
    adj_test_n = np.array([adj_test_orig[i].toarray() for i in range(adj_test_n.shape[0])])
    dist_train_e = []
    for i in range(adj_train_n.shape[0]):
      dist_train_e.append(squareform(pdist(data["coord_train"][i]))*adj_train_n[i])
    data["dist_train_e"] = np.array(dist_train_e)
    dist_test_e = []
    for i in range(adj_test_n.shape[0]):
      dist_test_e.append(squareform(pdist(data["coord_test"][i]))*adj_test_n[i])
